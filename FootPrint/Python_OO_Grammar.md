...
2. 多线程
   ...
   > 1. 程序启动时，自动创建并运行一个默认的主线程
   > 2. 当需要多个任务并行时，可使用  `threading.Thread ` 创建一个或多个辅助线程
   > 3. 辅助线程需要用  `threading.Thread` 来调用，但是主线程不需要

   终止程序

   > 1. 第1次 `Control + c` ，会终止主线程
   > 2. 第2次 `Control + c` ，会终止所有辅助线程
   ...
   def show_image_popup(image_path):
       ...
       confirm_button.clicked.connect(continue_program)
       cancel_button.clicked.connect(exit_program)
   ...
   def main():
       ...
       pic_location = Pic_Location(pic, confidence=0.7, max_times=2)
   ...
   if __name__ == "__main__":
       ...
       # 创建辅助线程：执行防故障措施
       thread = threading.Thread(target=PAU.Anti_Fault)
       # 设置辅助线程为守护线程，使其随主线程退出
       thread.daemon = True
       # 启动线程
       thread.start()
