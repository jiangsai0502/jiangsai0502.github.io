...
1. 向ChatGPT循环提交请求并获取请求结果
   ...
   >     
   >     SaiPage.wait_for_timeout(random.randint(5000,7000))
   ...
   >             print(text1)
   >     
   >     SaiPage.wait_for_timeout(random.randint(5000,8000))
   >     SaiContext.close()
   >     SaiBrowser.close()
   > # ---------------------收尾工作---------------------
   > 
...
* 滚动加载 + 爬取二级页信息
  ...
  >             NotEnd = False
  > 
  > def ExtractInfo(WritePage, MDdir, Xpath_Answer, Xpath_Answer_Desc, Xpath_Questions_Block, Xpath_Questions, Xpath_Author, Xpath_Question):
  >     # 创建MD文件
  >     os.chdir(MDdir)
  >     MarkDownMaker = html2text.HTML2Text()
  >     MarkDownMaker.ignore_links = True
  ...
  >     Xpath_Search_Result_url = '//meta[@itemprop="url"]'
  >     Elements = SaiPage.query_selector_all(Xpath_Search_Results)
  >     for element in Elements:
  >         # 判断节点是否有URL，有就提取出链接
  >         if element.query_selector(Xpath_Search_Result_url) is not None:
  >             elementUrl = element.query_selector(Xpath_Search_Result_url).get_attribute('content')
  >             SonPage = SaiContext.new_page()
  ...
  >             Xpath_Author = '//div[@class="AuthorInfo-head"]'
  >             Xpath_Question = '//span[@class="RichText ztext CopyrightRichText-richText css-117anjg"]'
  >             if SonPage.locator(Xpath_Answer).count() == 0:
  >                 SonPage.close()
  >             else:
  >                 # 滚动二级页获取更多数据
  >                 SaiScroll(SonPage, 2)
  >                 # 将二级页写入MarkDown
  >                 ExtractInfo(SonPage, MDdir, Xpath_Answer, Xpath_Answer_Desc, Xpath_Questions_Block, Xpath_Questions, Xpath_Author, Xpath_Question)
  >                 SonPage.close()
  >     # 收尾
  >     SaiContext.close()
  >     SaiBrowser.close()
...
* 翻页+提取一级页信息&二级页信息
  ...
  > 
  > # 翻页
  > def TurnPage(SaiPage, Xpath_Nav):
  >        # 所有导航数字集合
  >        All_Nav = set()
  >        # 已点击导航数字集合
  >        Clicked_Nav = set()
  ...
  >        SaiBrowser, SaiContext, SaiPage = Initialize(WebSite)
  >        # Xpath_Nav = '//div[@class="page-inner_2jZi2"]'
  >        Xpath_Nav = '//ul[@class="be-pager"]'
  >        TurnPage(SaiPage, Xpath_Nav)
  > 
  >        # 收尾
  >        SaiPage.close()
  >        SaiContext.close() 
